...
module.exports =
    activate: (state) ->
        ...
        atom.commands.add 'atom-text-editor', 'behat-it-atom:check_syntax', => @check_syntax
        atom.commands.add 'atom-workspace', 'behat-it-atom:launch-test', => @launch_test
    ...
    check_syntax: ->
      ...
      # editor = atom.workspace.getActiveEditor()
        ...
        if editor.getGrammar() == grammar
            args = ['--story-syntax',  '--lang it', editor.getPath()]
            output = (output) -> console.log(output)
            exitput = (code) -> console.log("exit code behat --story-syntax --lang it #{editor.getPath()}: #{code}")
